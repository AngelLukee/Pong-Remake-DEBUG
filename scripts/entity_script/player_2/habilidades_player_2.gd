...
func dash(player_2 : Player2):#HABILIADE TERMINADA E TESTADA
	
	if Input.is_action_pressed("down"):
		
		var tween = player_2.create_tween()
		var destino = clamp(player_2.position.y + 160, 96, 505)
		tween.tween_property(player_2, "position:y", destino, 0.15)
...
func clarao(flash_light : PointLight2D, player_2 : Player2) -> void:#Falta melhorias na luz
	
	var tween = player_2.create_tween()
	tween.tween_property(flash_light, "energy", 15.2, 0.4)
	
	await player_2.get_tree().create_timer(2.5).timeout
	var tween2 = player_2.create_tween()
	tween2.tween_property(flash_light, "energy", 0, 0.3)
	
	player_2.habilidade_ativa = false
	
func congelar(player_2 : Player2, player_1 : EntityPlayer, sound : Sounds) -> void:
	sound.PlayFreezeSound()
	
	var congelado_tween = player_1.create_tween()
	congelado_tween.tween_property(player_1.sprite_congelado, "modulate", Color.WHITE, 0.6)
	
	player_1.congelado = true
	await player_1.get_tree().create_timer(3.0).timeout
	
	var descongelado_tween = player_1.create_tween()
	descongelado_tween.tween_property(player_1.sprite_congelado, "modulate", Color.TRANSPARENT, 0.6)
	player_1.congelado = false
